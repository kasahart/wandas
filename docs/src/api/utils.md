...
For a DCASE/ASDKit-style directory layout, the example assumes `machine/split/filename.wav`. No DCASE knowledge is required; replace the resolver body with the naming rules used by your own files.
DCASE/ASDKit 形式の例では `machine/split/filename.wav` という規則を使います。DCASEの知識は不要で、自分のファイル命名規則にresolverの処理を置き換えられます。

```python
from pathlib import Path
import wandas as wd

def resolve_dcase(path: Path):
...
training_fans = dataset.select(machine="fan", split="train")
```
...
3. `frame.compute()`: materialize waveform samples. / 波形サンプルを実体化。
