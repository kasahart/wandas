...
The example below uses a simple `group/batch/filename.wav` directory layout. Replace the resolver body with the naming rules used by your own files.
次の例では、単純な `group/batch/filename.wav` というフォルダ規則を使います。自分のファイル命名規則にresolverの処理を置き換えられます。
