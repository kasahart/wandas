...
- `BaseFrame.cache()` provides an explicit synchronous boundary for reusing the
  computed data of a bounded in-memory Frame without changing lineage or Recipe/WDF
  schemas.
...
- [Issue #326](https://github.com/kasahart/wandas/issues/326): add the minimal
  no-argument `BaseFrame.cache()` API. It returns a new same-type Frame around one
  computed raw NumPy tensor, preserves calibration, metadata, axes, offsets,
  Frame-specific state, and lineage, and adds no cache manager, status flag,
  release API, eviction, scheduler selection, `persist()` alias, or Recipe node.
  Raw compute results represented as `np.ma.MaskedArray` are rejected explicitly
  because mask representation is not stable across supported xarray versions.
...
| --- | --- | --- | --- | --- | --- |
| `BaseFrame.cache()` | Stable (additive) | None | Use for bounded Frames that fit in local memory and need repeated materialization or downstream processing; use `.data` for a one-off NumPy value. Raw `np.ma.MaskedArray` compute results raise `ValueError`. | 0.7.0 | Minimal synchronous materialization boundary approved by [Issue #326](https://github.com/kasahart/wandas/issues/326); WDF and Recipe schemas remain unchanged. |
