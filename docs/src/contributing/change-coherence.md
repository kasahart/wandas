...
| --- | --- |
| One local owner, explicit acceptance, no public or architectural ambiguity / ownerが局所的でacceptanceが明確、公開・architecture上の曖昧さがない | Implement directly; no change record or independent review is required / 直接実装する。change recordやindependent reviewは不要 |
...
| Public behavior, architecture, ownership, compatibility, acceptance, or scope is ambiguous or materially moving / 公開behavior、architecture、ownership、compatibility、acceptance、scopeが曖昧またはmaterialに変動 | Maintain a structured change record and apply the replan boundary / 構造化change recordを維持し、replan boundaryを適用 |
...
decisions explicit. Use the structured record when the answers may evolve.
発火対象の変更を実装する前に、次の少数の判断を明示します。回答が変動しうる場合は
structured recordを使用します。
...
## Use the mechanical guardrail / 機械的guardrailを使う

The repository provides a generic JSON
[example record](https://github.com/kasahart/wandas/blob/main/.agents/skills/wandas-change-coherence/references/change-record.example.json)
and a [validator](https://github.com/kasahart/wandas/blob/main/scripts/validate_change_coherence.py).
The record is required only after this risk-based procedure is triggered. Keep it
as a task handoff artifact or commit it only when durable repository evidence is
useful.
repositoryはgenericなJSON example recordとvalidatorを提供します。recordはrisk-based procedureが
発火した場合だけ必要です。task handoff artifactとして保持し、durableなrepository evidenceが
有用な場合だけcommitします。

```bash
uv run python scripts/validate_change_coherence.py <record.json>
uv run python scripts/validate_change_coherence.py <record.json> --expect REVIEW_READY
```

The validator derives one of these outcomes from structure and state, rather
than accepting a prose readiness claim:
validatorはproseのreadiness claimを受け入れるのではなく、構造とstateから次を導出します。

| Outcome | Meaning / 意味 |
| --- | --- |
| `DIRECT` | A small, explicit local change may proceed directly / 小さく明確な局所変更を直接実行できる |
| `SIBLING_SEARCH_REQUIRED` | A finding lacks its bounded family search / findingにbounded family searchがない |
| `FIX_REQUIRED` | The contract is stable and bounded implementation or evidence work remains / contractはstableでboundedな実装またはevidence作業が残る |
| `CONTRACT_REPLAN_REQUIRED` | A material contract boundary is unstable or stale / materialなcontract boundaryが不安定または古い |
| `REVIEW_READY` | Current-head contract, findings, alignment, tracking, and evidence are coherent / current-headのcontract、finding、alignment、tracking、evidenceが整合している |

Validation cannot decide the product contract or discover siblings automatically.
It guarantees that those judgments are explicit, current, and consistent before
the workflow claims review readiness.
validatorはproduct contractを決定したりsiblingを自動発見したりはできません。workflowが
review readinessを主張する前に、それらの判断が明示的、最新、一貫していることを保証します。
