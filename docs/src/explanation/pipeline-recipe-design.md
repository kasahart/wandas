...
`fft()`、`stft()`、`ifft()`、`istft()`、`welch()`、`coherence()`、`csd()`、`transfer_function()` のように、出力 frame class が変わる method を replay する。戻り型は入力 frame と同じとは限らないため、`RecipeSpec.apply(frame)` の戻り値は実行した step chain に従う。
...
- typed method step は明示 allowlist に含まれる method だけを抽出する。現在は `fft`、`stft`、`ifft`、`istft`、`welch`、`coherence`、`csd`、`transfer_function` に限定する。
...
- typed method extraction を `noct_spectrum` へ広げる。
