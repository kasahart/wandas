...
`fft()`、`stft()`、`ifft()`、`istft()` のように、出力 frame class が変わる method を replay する。戻り型は入力 frame と同じとは限らないため、`RecipeSpec.apply(frame)` の戻り値は実行した step chain に従う。
...
- typed method step は明示 allowlist に含まれる method だけを抽出する。現在は `fft`、`stft`、`ifft`、`istft` に限定する。
...
- typed method extraction を `welch`、`noct_spectrum`、cross-channel spectral transform へ広げる。
