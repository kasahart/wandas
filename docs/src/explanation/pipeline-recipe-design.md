...
`GraphRecipeSpec` は明示的に組む API である。`GraphRecipeSpec.from_frame(processed, input_names=(...))` は root が frame-frame `+` または `add_with_snr` で、親が 2 本の linear chain として表現できる場合だけ抽出する。入力名は lineage から推定せず、呼び出し側が渡す。
