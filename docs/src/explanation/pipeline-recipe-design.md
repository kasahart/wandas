...
`frame[0:2]` のような channel slice、`frame[[1, 0]]` のような integer list selection、`frame[["left", "right"]]` のような label list selection、`frame[:, 100:400]` のような slice-only tuple indexing を replay する。Recipe は selection logic や metadata 更新を複製せず、保存した key で既存 `frame[key]` を呼ぶ。NumPy array、boolean mask、point/fancy time selection、regex/callable/dict query は、indexing intent を保存する schema が必要なため対象外である。
...
- `__getitem__` は channel slice、integer list、label list、slice-only tuple indexing だけを `IndexingStep` として抽出する。array indexing、boolean mask、point/fancy time selection は indexing intent と source time offset contract の追加 schema が必要なため現在は拒否する。
