...
`frame.apply(custom_func, output_shape_func=..., **params)` のうち、同じ frame class を返す importable custom function を replay する。対象は module-level function に限定し、lambda、closure、nested function、callable object、bound method、`functools.partial`、`__main__` 上の関数は抽出しない。`output_shape_func` も指定する場合は同じく importable module-level function に限定する。
...
- `frame.apply(...)` の custom callable は、同じ frame class を返し、function と `output_shape_func` が importable module-level function として検証できる場合だけ `CustomFunctionStep` に抽出する。lambda、closure、nested function、callable object、bound method、`functools.partial`、`__main__` 上の関数、custom domain transition は拒否する。
...
- custom domain transition を含む recipe の型制約。
