...
`sum()`、`mean()`、`fix_length()`、`channel_difference()`、`get_channel()`、`remove_channel()` のように、metadata 変換を frame method 側に持つ処理を replay する。Recipe 側では metadata 再構成を複製せず、既存 method を呼ぶ。
...
- `add_channel()` は外部データ入力を含むため graph recipe の範囲として扱う。`rename_channels()` は mapping intent の schema を定義してから扱う。
- v1 では複数入力 operation を recipe から呼ばない。必要な場合は operation-specific method を使う。
- method-aware step は明示 allowlist に含まれる method だけを抽出する。現在は `get_channel`、`remove_channel`、`fix_length`、`sum`、`mean`、`channel_difference` に限定する。
