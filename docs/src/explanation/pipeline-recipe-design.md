...
Wandas 側に小さな replayable spec を置き、sklearn は optional adapter として扱う。中心は `OperationSpec` と `RecipeSpec` であり、実行は既存の `Frame.apply_operation(...)`、frame method、frame operator に委譲する。これにより、frame immutability、metadata propagation、lineage-derived history view、Dask lazy execution は既存 frame operation の契約に従う。
