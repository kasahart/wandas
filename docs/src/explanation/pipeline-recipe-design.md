...
- `__getitem__` は channel slice、integer list/ndarray、label list、slice-only tuple indexing だけを `IndexingStep` として抽出する。boolean mask、point/fancy time selection は indexing intent と source time offset contract の追加 schema が必要なため現在は拒否する。
