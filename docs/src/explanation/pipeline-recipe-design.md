...
`frame[0:2]` のような channel slice と `frame[["left", "right"]]` のような label list selection を replay する。Recipe は selection logic や metadata 更新を複製せず、保存した key で既存 `frame[key]` を呼ぶ。tuple indexing、time slicing、NumPy array、boolean mask、regex/callable/dict query は、indexing intent を保存する schema が必要なため対象外である。
...
- `__getitem__` は channel slice と label list だけを `IndexingStep` として抽出する。tuple indexing、time slicing、array indexing、boolean mask は indexing intent の schema が必要なため現在は拒否する。
