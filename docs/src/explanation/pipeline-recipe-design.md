...
- `steps: tuple[OperationSpec | MethodStep, ...]`
- `apply(frame) -> frame`
- `from_frame(frame) -> RecipeSpec`

`steps` を順番に処理し、`OperationSpec` は `frame.apply_operation(step.operation, **step.params)`、`MethodStep` は対応する frame method を呼ぶ。`RecipeSpec` 自体は frame を保持しないため、同じレシピを複数 frame に再適用できる。
