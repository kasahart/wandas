...
`frame[0:2]` のような channel slice、`frame[["left", "right"]]` のような label list selection、`frame[:, 100:400]` のような slice-only tuple indexing を replay する。Recipe は selection logic や metadata 更新を複製せず、保存した key で既存 `frame[key]` を呼ぶ。NumPy array、boolean mask、integer-list channel selector を含む tuple indexing、point/fancy time selection、regex/callable/dict query は、indexing intent を保存する schema が必要なため対象外である。
