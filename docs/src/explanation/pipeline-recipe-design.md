...
`fft()`、`stft()`、`ifft()`、`istft()`、`welch()`、`noct_spectrum()`、`noct_synthesis()`、`coherence()`、`csd()`、`transfer_function()` のように、出力 frame class が変わる method を replay する。戻り型は入力 frame と同じとは限らないため、`RecipeSpec.apply(frame)` の戻り値は実行した step chain に従う。
...
- typed method extraction を psychoacoustic result frames へ広げる。
