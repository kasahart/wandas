...
Time-continuous processing normally uses one time chunk per channel. WDF 0.4 passes
source chunks to its writer without precomputing the complete tensor; storage and
loaded Dask chunk topology may differ. See the [scalability contract](scalability-contract.md)
for the exact limits and benchmark command.
