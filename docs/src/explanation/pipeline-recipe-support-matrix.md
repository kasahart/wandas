...
| channel を選ぶ | 一部対応 | `RecipeSpec` | `frame[0:2]`, `frame[["left", "right"]]`, `get_channel(query="left")` | 名前、範囲、番号の一覧、True/False の一覧は対応 |
...
| WDF に処理履歴を残す | 対応 | operation summaries snapshot | `save(...).load(...)` | 表示用履歴だけ。executable Recipe は保存しない |
| WDF に Recipe を保存する | 非対応 | なし | `save(...).load(...)` | executable Recipe persistence は #257 で決める |
| sklearn 形式で Wandas の処理を使う | 一部対応 | `wandas.pipeline.sklearn` | `Pipeline([...])` | 単純な operation transformer だけ。詳しくは [Recipe interoperability/export boundaries](pipeline-recipe-interoperability-export.md) |
| joblib/skops に export する | 非対応 | なし | `joblib.dump(...)` | Python object serialization は安定した Recipe schema ではない |
...
| 入力が複数、または `add_channel` や array 入力がある | `NodeGraphRecipeSpec.from_frame(processed, input_names=(...))`。同じ元データを2回使う場合も各leafには異なる名前を付け、`apply`時に両方へ同じframeを渡す |
...
- true DAG identity は #270 で扱う。今の `NodeGraphRecipeSpec` は、同じ branch が保存された Recipe の中で2回出ることがある。
- executable Recipe WDF persistence は #257 に残す。
