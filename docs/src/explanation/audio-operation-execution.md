...
| `fft` / `FFT` | D | Formal 8-channel `Frame._compute` materialization improved 216.1→163.8 ms, but peak RSS increased 608.9→814.5 MiB. | Whole-frame | Keep; output/temporary growth outweighs the timing observation. |
| `ifft` / `IFFT` | D | Formal 8-channel `Frame._compute` materialization improved 194.2→176.0 ms, but peak RSS increased 623.8→845.0 MiB. | Whole-frame | Keep; the material memory regression is not acceptable. |
| `istft` / `ISTFT` | D | Formal 8-channel `Frame._compute` materialization regressed 97.4→194.7 ms with essentially flat peak RSS (454.3→454.1 MiB). | Whole-frame | Keep. |
...
| `cepstrum` / `Cepstrum` | D | Formal 8-channel `Frame._compute` materialization improved 312.7→242.6 ms, but peak RSS increased 746.1→867.7 MiB. | Whole-frame | Keep; the memory regression outweighs the timing improvement. |
| `lifter` / `Lifter` | D | Formal 8-channel `Frame._compute` materialization improved 33.3→29.9 ms, but peak RSS increased 448.2→509.5 MiB. | Whole-frame | Keep; material memory regression outweighs the small timing observation. |
| `spectral_envelope` / `SpectralEnvelope` | D | Formal 8-channel `Frame._compute` materialization improved 214.2→139.2 ms, but peak RSS increased 615.8→738.7 MiB. | Whole-frame | Keep. |
| `spectrogram_cepstrum` / `SpectrogramCepstrum` | D | Formal 8-channel `Frame._compute` materialization improved 93.1→48.0 ms, but peak RSS increased 557.2→645.5 MiB. | Whole-frame | Keep; the rank-3 timing benefit does not offset the material memory regression. |
| `noct_synthesis` / `NOctSynthesis` | D | Formal 8-channel `Frame._compute` materialization regressed 1.876→6.102 s and peak RSS increased 486.1→897.7 MiB. | Whole-frame | Keep; spectrum adoption does not change shared synthesis behavior. |
| `fix_length` / `FixLength` | D | Padding/slicing is cheap. Formal 8-channel `Frame._compute` materialization improved 39.5→31.0 ms, but peak RSS increased 448.8→509.6 MiB. | Whole-frame | Keep; the memory regression outweighs this small transform's timing. |
| `fade` / `Fade` | D | Multiplication by one shared full-length envelope is cheap. Formal 8-channel `Frame._compute` materialization regressed 40.8→43.5 ms while peak RSS increased 471.6→631.3 MiB. | Whole-frame | Keep. |
| `loudness_zwtv` / `LoudnessZwtv` | D | Formal 8-channel `Frame._compute` materialization regressed 6.094→9.361 s and peak RSS increased 235.5→495.9 MiB. | Whole-frame | Keep; optional MoSQITo call/task overhead dominates. |
| `roughness_dw` / `RoughnessDw` | D | Formal 8-channel `Frame._compute` materialization improved 1.411→1.215 s, but peak RSS increased 191.2→257.7 MiB. | Whole-frame | Keep. |
| `sharpness_din` / `SharpnessDin` | D | Formal 8-channel `Frame._compute` materialization regressed 6.128→9.304 s and peak RSS increased 235.4→476.9 MiB. | Whole-frame | Keep. |
