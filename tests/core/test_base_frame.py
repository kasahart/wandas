...
def test_explicit_source_time_range_is_preserved_by_create_new_instance() -> None:
    frame = ConcreteFrame(
        da.from_array(np.arange(8).reshape(1, 8)),
        sampling_rate=4.0,
        source_time_offset=10.0,
    )
    frame._xr.attrs["source_time_range"] = (10.0, 11.0)
...
class TestBaseFrameIndexing:
    ...
    def test_getitem_with_tuple_time_slice_advances_source_time_offset(self) -> None:
        frame = ChannelFrame(
            da_from_array(np.arange(400, dtype=float).reshape(1, 400), chunks=(1, -1)),
            sampling_rate=100.0,
        ...
        result = frame[:, 100:200]

        assert result.source_time_offset == pytest.approx(6.0)
