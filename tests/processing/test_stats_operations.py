...
class TestCrestFactor:
    """Tests for the CrestFactor audio operation."""
    ...
    def setup_method(self) -> None:
        ...
        # Pure sine wave: crest factor = sqrt(2)
        t = np.linspace(0, 1, self.sample_rate, endpoint=False)
        sine: NDArrayReal = np.sin(2 * np.pi * 440 * t).astype(np.float64)

        # Mono (1, samples) and stereo (2, samples) signals
        self.signal_mono: NDArrayReal = np.array([sine])
        self.signal_stereo: NDArrayReal = np.array([sine, sine * 0.5])

        # All-zeros signal
        self.signal_zeros: NDArrayReal = np.zeros((2, self.sample_rate))

        self.dask_mono: DaArray = da.from_array(self.signal_mono, chunks=(1, -1))
        self.dask_stereo: DaArray = da.from_array(self.signal_stereo, chunks=(1, -1))
        self.dask_zeros: DaArray = da.from_array(self.signal_zeros, chunks=(1, -1))

    # ------------------------------------------------------------------
    # Pillar 1 – Immutability: input data is not modified
    # ------------------------------------------------------------------

    def test_immutability(self) -> None:
        """Processing must not mutate the input array."""
        original = self.signal_mono.copy()
        _ = self.cf_op.process(self.dask_mono).compute()
        np.testing.assert_array_equal(self.signal_mono, original)

    # ------------------------------------------------------------------
    # Pillar 2 – Metadata sync: output shape is (channels, 1)
    # ------------------------------------------------------------------

    def test_output_shape_mono(self) -> None:
        """Output of a mono signal should be (1, 1)."""
    ...
    def test_output_shape_stereo(self) -> None:
        """Output of a stereo signal should be (2, 1)."""
        result = self.cf_op.process(self.dask_stereo).compute()
        assert result.shape == (2, 1), f"Expected (2, 1), got {result.shape}"
    ...
    def test_dc_signal_crest_factor(self) -> None:
        """A constant (DC) signal has crest factor = 1.0."""
        dc: NDArrayReal = np.ones((1, self.sample_rate))
        dask_dc = da.from_array(dc, chunks=(1, -1))
        result = self.cf_op.process(dask_dc).compute()
        np.testing.assert_allclose(result[0, 0], 1.0, rtol=1e-10)

    # ------------------------------------------------------------------
    # Pillar 4 – Reference-based verification: zero-signal edge case
    # ------------------------------------------------------------------

    def test_zero_signal_returns_nan(self) -> None:
        """All-zero input must produce NaN (zero-division handled gracefully)."""
        result = self.cf_op.process(self.dask_zeros).compute()
    ...
    def test_operation_registry(self) -> None:
        """CrestFactor must be reachable via the operation registry."""
        assert get_operation("crest_factor") is CrestFactor
    ...
    def test_display_name(self) -> None:
        """Display name must be 'crest_factor'."""
