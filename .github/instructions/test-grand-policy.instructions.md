...
- **`pure_sine_1khz`**: 1 kHz 純音（SR=16000 Hz、1秒間）。FFT ピーク位置が解析的に予測可能。
- **`dual_tone`**: 50 Hz + 1000 Hz の合成音（SR=16000 Hz）。フィルタ減衰量の検証に使用。
- **`calibrated_sine_1khz_70dB`**: 70 dB SPL の 1 kHz 純音（SR=48000 Hz）。IEC 規格準拠の心理音響テスト標準信号。基準音圧 p_ref=2e-5 Pa に基づいて振幅を計算すること。
- **`impulse`**: 単位インパルス（SR=16000 Hz）。フィルタのインパルス応答検証に使用。

Processing 層の fixture（`_dask` suffix）は `(DaskArray, int)` のタプルで、`da_from_array(data.reshape(1, -1), chunks=(1, -1))` を使って channel-wise chunks で作成すること。
...
`operation_history` に処理内容とパラメータが記録されていること。演算後に `operation_history` の件数が 1 件増加し、最後のエントリに正しい **レジストリキー**（例: `"lowpass_filter"`）とパラメータが含まれていることを検証すること。
