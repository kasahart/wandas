...
`conftest.py` に以下の fixture を定義すること。信号は乱数ではなく決定論的な正弦波を使用すること。

- **`channel_frame`**: 標準的な 2 チャンネルフレーム（440 Hz と 880 Hz の正弦波、SR=16000 Hz）。ラベルは `["ch0", "ch1"]` とする。
- **`mono_frame`**: 単チャンネルフレーム（440 Hz 正弦波、SR=16000 Hz）。

不変性・構造テストで信号内容が関係ない場合は、シード固定乱数（`np.random.default_rng(42)`）も許容されるが、正弦波信号を優先すること。
...
- **MoSQITo 等価性**: 外部ライブラリとの結果一致を検証する（`rtol=0.01`）
