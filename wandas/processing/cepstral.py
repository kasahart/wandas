...
class Cepstrum(AudioOperation[NDArrayReal, NDArrayReal]):
    ...
    def process(self, data: da.Array) -> da.Array:
        logger.debug("Adding delayed cepstrum operation to computation graph")
        delayed_result = delayed(self._create_named_wrapper(), pure=self.pure)(data)
        output_shape = self.calculate_output_shape(data.shape)
        return da.from_delayed(delayed_result, shape=output_shape, dtype=_real_output_dtype(np.dtype(data.dtype)))
...
class Lifter(AudioOperation[NDArrayReal, NDArrayReal]):
    ...
    def process(self, data: da.Array) -> da.Array:
        logger.debug("Adding delayed lifter operation to computation graph")
        delayed_result = delayed(self._create_named_wrapper(), pure=self.pure)(data)
        output_shape = self.calculate_output_shape(data.shape)
        return da.from_delayed(delayed_result, shape=output_shape, dtype=_real_output_dtype(np.dtype(data.dtype)))
