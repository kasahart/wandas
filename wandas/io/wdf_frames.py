...
def encode_frame_state(frame: BaseFrame[Any]) -> dict[str, Any]:
    """Encode the typed Frame portion of the WDF schema.

    The ordered xarray dimension names are persisted separately from coordinate
    values: dimensions identify tensor semantics, while represented-axis coordinates
    are handled by :func:`frame_dimension_coordinates`.
    """
...
def _coordinate_grid(frame: BaseFrame[Any], name: str) -> tuple[float, float | None]:
    """Return the canonical sampling grid for a persisted represented axis.

    Only axes whose slice position is part of their formal contract are registered
    here. ``SpectralFrame`` frequency and ``SpectrogramFrame`` time axes are derived
    from constructor state and therefore are not persisted as independent
    coordinates.
    """
    typed = cast(Any, frame)
    if name == "quefrency":
