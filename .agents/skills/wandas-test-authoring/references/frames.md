...
**Prerequisite**: Read the [grand policy](grand-policy.md) first.
...
Verify that the sampling rate is preserved after the operation. For methods designed to record provenance, verify that runtime lineage is extended and that the derived `operation_history` count increases by exactly 1. Structural operations follow their declared Recipe contract: `rename_channels()` must extend lineage and derived `operation_history` by one node so a following operation can select the renamed channel during replay.
...
## Cross-References
- [Grand policy](grand-policy.md) — 4 pillars and test pyramid
- [Frame and Operation extension guide](../../../../docs/src/contributing/frame-operation-extensions.md) — Frame architecture and immutability rules
